...
class Solution(object):
    def mostCommonWord(self, paragraph, banned):
        ...
        """
        punc = set({',', ';', ":", ".", "!","?","'"})
        p = "".join(ch if ch not in punc else " " for ch in paragraph)
        p = re.sub(" +", " ", p)
        d = Counter(p.lower().strip().split(" "))
        fre = 0
        res = ""
        print(p)
        for w in d:
            # print(w)
            if (d[w] > fre and w not in banned):
                res = w
                fre = d[w]
        return res
    ...
    def mostCommonWord(self, paragraph, banned):
        ...
        d = Counter(paragraph.lower().strip().split(" "))
        b = set(banned)
        fre = 0
        for w in d:
            if (d[w] > fre and w not in b):
                o = w
                fre = d[w]
        return o
s=Solution()
