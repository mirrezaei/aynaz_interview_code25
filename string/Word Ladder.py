...
#the idea is to create a graph that nodes are words in wordList, and edges indicate if it is possible to reach from one to another just by changing one char
#run bfs on the graph from end and see how many steps it takes to reach the start
class Solution2(object):
    ...
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        """
        # create graph
        d = {}#shows wich words are reachable from each other
        d[beginWord] = [set(),float('inf')]
        d[endWord] = [set(),0]
        for w in wordList:
            d[w] = [set(),float('inf')]
        same={}
        for w1 in (wordList + [beginWord]):#for each char in a word, replace it by *, and use it as the key of dict
            for i in range(len(w1)):
                if(w1[:i]+"*"+w1[i+1:] not in same):
                    same[w1[:i]+"*"+w1[i+1:]]=set()
                same[w1[:i] + "*" + w1[i + 1:]].add(w1)

        for key in same:#create the graph
            l=list(same[key])
        ...
        # bfs traverse
        h=[]
        heapq.heappush(h,(0,endWord))
        return (self.bfs(d, h, beginWord))
...
class Solution(object):
    ...
    def bfs(self,d, h, beg):
        while(len(h)>0):
            node=heapq.heappop(h)
            cur=node[0]#current distance with end node
            node=node[1]# current node
            if(node==beg):
                return cur+1
            for nei in d[node][0]:
                if(d[nei][1]>cur+1):
